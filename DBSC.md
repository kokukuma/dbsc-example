...
DBSC is a web authentication mechanism that enhances security by cryptographically binding session credentials to specific devices, making stolen cookies useless to attackers without physical access to the device.

## Overview

Device Bound Session Credentials (DBSC) is a browser feature that:
...
This technology significantly improves web security by mitigating session hijacking attacks.

## Key Components

### 1. Device-bound Keypair

- **Private Key**: Stored securely in the device's Trusted Platform Module (TPM) or Secure Enclave
- **Public Key**: Sent to the server during session establishment
- **Key Use**: Challenge-response authentication without exposing the private key

### 2. HTTP Headers

- **`Sec-Session-Registration`**: Server → Browser (initiates DBSC with a challenge)
- **`Sec-Session-Response`**: Browser → Server (signed challenge response)
- **`Sec-Session-Id`**: Browser → Server (identifies the session during refresh)
- **`Sec-Session-Challenge`**: Server → Browser (issues new challenge during refresh)

### 3. Short-lived Session Cookies

- **Cookie Duration**: Typically 10 minutes (vs. traditional days/weeks)
- **Auto Refresh**: Browser automatically refreshes cookies in the background
- **Device Binding**: Refreshes only work from the original device

### 4. Challenge-Response Protocol

- **Server Challenge**: Random nonce sent to browser
- **Browser Response**: JWT containing the challenge, signed by device private key
- **Server Verification**: Verifies signature using stored public key

## Authentication Flow

1. **Initial Login**: User authenticates with username/password
2. **DBSC Registration**:
   - Server sends `Sec-Session-Registration` header with challenge
   - Browser generates device-bound keys (if not already existing)
   - Browser signs challenge with private key
   - Browser sends public key and signed response
   - Server verifies signature and stores public key with session
   - Server issues short-lived cookies

3. **Session Refresh**:
   - When cookies near expiration, browser requests refresh
   - Server sends new challenge via `Sec-Session-Challenge` header
   - Browser signs new challenge with device private key
   - Server verifies signature matches stored public key
   - Server issues new short-lived cookies

## Security Benefits

1. **Phishing Resistance**: Even if cookies are stolen, they can't be used on attacker's device
2. **Cross-device Protection**: Session only works on the authenticated device
3. **Session Hijacking Mitigation**: Short-lived cookies reduce attack window
4. **Transparent UX**: Automatic refreshes happen without user interaction

## Browser Support

- **Supported Browsers**: Chrome 115+ with experimental flag enabled
- **Flag**: `chrome://flags/#enable-standard-device-bound-session-credentials` 
- **Cryptography**: ES256 (ECDSA with P-256 curve and SHA-256)
