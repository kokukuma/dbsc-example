...
// CreateSessionRegistrationToken creates a JWT token for DBSC session registration
func CreateSessionRegistrationToken(privateKey *ecdsa.PrivateKey, username string, challenge string) (string, error) {
	now := time.Now()
	...
	claims := jwt.RegisteredClaims{
		...
		ExpiresAt: jwt.NewNumericDate(now.Add(time.Duration(ChallengeMaxAge) * time.Second)),
	}
...
// VerifyClientToken verifies a JWT token from the client
func VerifyClientToken(tokenString string, publicKeyPEM string, expectedChallenge string) (bool, string, error) {
